...
public class FilesStorageServiceImpl implements FilesStorageService {
	  ...
	  public void save(MultipartFile file, String dirName) {
	    try {
	    	Path path = createIfNotExists(dirName);
	  ...
	  public Resource load(String dirName, String filename) {
	    try {
	      Path file = root.resolve(dirName).resolve(filename);
	  ...
	  public boolean delete(String dirName, String filename) {
	    try {
	      ...
	      Path file = root.resolve(dirName);
	      Path fil = file.resolve(filename);
	      return Files.deleteIfExists(fil);
	    } catch (IOException e) {
	      throw new RuntimeException("Error: " + e.getMessage());
	    }
	  }
